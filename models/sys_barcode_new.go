...
func (t *SysBarcodeNew) TableName() string {
	return "sys_barcode_new"
}

func init() {
	orm.RegisterModel(new(SysBarcodeNew))
}

// AddSysBarcodeNew insert a new SysBarcodeNew into database and returns
// last inserted Id on success.
func AddSysBarcodeNew(m *SysBarcodeNew) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetSysBarcodeNewById retrieves SysBarcodeNew by Id. Returns error if
// Id doesn't exist
func GetSysBarcodeNewById(id int) (v *SysBarcodeNew, err error) {
	o := orm.NewOrm()
	v = &SysBarcodeNew{Id: id}
...
// GetAllSysBarcodeNew retrieves all SysBarcodeNew matches certain condition. Returns empty list if
// no records exist
func GetAllSysBarcodeNew(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(SysBarcodeNew))
	// query k=v
	for k, v := range query {
...
// UpdateSysBarcodeNew updates SysBarcodeNew by Id and returns error if
// the record to be updated doesn't exist
func UpdateSysBarcodeNewById(m *SysBarcodeNew) (err error) {
	o := orm.NewOrm()
	v := SysBarcodeNew{Id: m.Id}
...
// DeleteSysBarcodeNew deletes SysBarcodeNew by Id and returns error if
// the record to be deleted doesn't exist
func DeleteSysBarcodeNew(id int) (err error) {
	o := orm.NewOrm()
	v := SysBarcodeNew{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&SysBarcodeNew{Id: id}); err == nil {
