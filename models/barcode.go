...
func (t *Barcode) TableName() string {
	return "barcode"
}

func init() {
	orm.RegisterModel(new(Barcode))
}

// AddBarcode insert a new Barcode into database and returns
// last inserted Id on success.
func AddBarcode(m *Barcode) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetBarcodeById retrieves Barcode by Id. Returns error if
// Id doesn't exist
func GetBarcodeById(id int) (v *Barcode, err error) {
	o := orm.NewOrm()
	v = &Barcode{Id: id}
...
// GetAllBarcode retrieves all Barcode matches certain condition. Returns empty list if
// no records exist
func GetAllBarcode(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Barcode))
	// query k=v
	for k, v := range query {
...
// UpdateBarcode updates Barcode by Id and returns error if
// the record to be updated doesn't exist
func UpdateBarcodeById(m *Barcode) (err error) {
	o := orm.NewOrm()
	v := Barcode{Id: m.Id}
...
// DeleteBarcode deletes Barcode by Id and returns error if
// the record to be deleted doesn't exist
func DeleteBarcode(id int) (err error) {
	o := orm.NewOrm()
	v := Barcode{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Barcode{Id: id}); err == nil {
