...
func (t *SysBarcode) TableName() string {
	return "sys_barcode"
}

func init() {
	orm.RegisterModel(new(SysBarcode))
}

// AddSysBarcode insert a new SysBarcode into database and returns
// last inserted Id on success.
func AddSysBarcode(m *SysBarcode) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetSysBarcodeById retrieves SysBarcode by Id. Returns error if
// Id doesn't exist
func GetSysBarcodeById(id string) (v *SysBarcode, err error) {
	o := orm.NewOrm()
	v = &SysBarcode{Id: id}
...
// GetAllSysBarcode retrieves all SysBarcode matches certain condition. Returns empty list if
// no records exist
func GetAllSysBarcode(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(SysBarcode))
	// query k=v
	for k, v := range query {
...
// UpdateSysBarcode updates SysBarcode by Id and returns error if
// the record to be updated doesn't exist
func UpdateSysBarcodeById(m *SysBarcode) (err error) {
	o := orm.NewOrm()
	v := SysBarcode{Id: m.Id}
...
// DeleteSysBarcode deletes SysBarcode by Id and returns error if
// the record to be deleted doesn't exist
func DeleteSysBarcode(id string) (err error) {
	o := orm.NewOrm()
	v := SysBarcode{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&SysBarcode{Id: id}); err == nil {
